...
public class TetrisGUI extends JPanel implements Observer {
    ...
    private void endGame() {
        if (myGameStatus) {
            myGameStatus = false;
            pause();
            myActionMap.get(RESUME).setEnabled(false);
    ...
    private void newGame() {
        if (myGameStatus) {
            ...
            myActionMap.get(MOVE_DOWN).setEnabled(true);
            myActionMap.get(RESUME).setEnabled(true);
            myGameStatus = true;
            myGamePanel.setStatus(myGameStatus); 
            myScoreInfo.newGameReset();
        }
    ...
    private void setupGameInfoPanel() {
        ...
        final TitledBorder title = BorderFactory.createTitledBorder(blackline, 
                        "Game Information [n = Current Level]", 
    ...
    private void setupExtraBar() {
        ...
        //Level Select
        final JMenuItem levelItem = new JMenuItem("Set Level...");
        extraMenu.add(levelItem);
    ...
    /**
     * Shows a dialog to set level.
     */
    private void setLevelDialog() {
        final JFrame levelFrame = new JFrame("Set Level");
        final JPanel levelDialog = new JPanel(new FlowLayout());
        levelFrame.add(levelDialog);
        levelFrame.setLocationRelativeTo(null);
        levelFrame.setResizable(false);
        final JLabel levelLabel = new JLabel("Set Level:");
        final JSlider levelSlider = new JSlider(1, 9, 1);
        levelSlider.setMajorTickSpacing(1);
        levelSlider.setPaintTicks(true);
        levelSlider.setPaintLabels(true);
        final JButton levelSet = new JButton("Confirm");
        levelSet.setDefaultCapable(true);
        levelFrame.getRootPane().setDefaultButton(levelSet);
        levelFrame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
        levelDialog.add(levelLabel);
        levelDialog.add(levelSlider);
        levelDialog.add(levelSet);
        levelSet.addActionListener(e -> {
            final int levelToSet = levelSlider.getValue();
            myCurrentLevel = levelToSet;
            myScoreInfo.setLevel(levelToSet);
            levelFrame.dispose();
        });
        levelFrame.pack();
